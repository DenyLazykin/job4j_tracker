package ru.job4j.optional.optionalfind;

import org.junit.Test;

import java.util.Map;
import java.util.Optional;
import java.util.Set;

import static org.assertj.core.api.Assertions.assertThat;

public class CollageTest {
    @Test
    public void whenAccountIsOptionalEmpty() {
        Map<Student, Set<Subject>> students = Map.of(
                new Student("Student1", "000001", "201-18-15"),
                Set.of(new Subject("Math", 70),
                        new Subject("English", 85)),
                new Student("Student2", "000002", "201-18-15"),
                Set.of(new Subject("Economic", 75),
                        new Subject("Sociology", 65))
        );
        Collage collage = new Collage(students);
        assertThat(collage.findByAccount("000010")).isEqualTo(Optional.empty());
    }

    @Test
    public void whenAccountIsOptionalNotEmpty() {
        Map<Student, Set<Subject>> students = Map.of(
                new Student("Student1", "000001", "201-18-15"),
                Set.of(new Subject("Math", 70),
                        new Subject("English", 85)),
                new Student("Student2", "000002", "201-18-15"),
                Set.of(new Subject("Economic", 75),
                        new Subject("Sociology", 65))
        );
        Collage collage = new Collage(students);
        assertThat(collage.findByAccount(
                "000001").get().getGroup()).isEqualTo("201-18-15");
    }

    @Test
    public void whenSubjectIsOptionalEmptyWithNotFoundAccount() {
        Map<Student, Set<Subject>> students = Map.of(
                new Student("Student1", "000001", "201-18-15"),
                Set.of(new Subject("Math", 70),
                        new Subject("English", 85)),
                new Student("Student2", "000002", "201-18-15"),
                Set.of(new Subject("Economic", 75),
                        new Subject("Sociology", 65))
        );
        Collage collage = new Collage(students);
        assertThat(collage.findBySubjectName(
                "000010", "Sociology")).isEqualTo(Optional.empty());
    }

    @Test
    public void whenSubjectIsOptionalEmptyWithFoundAccount() {
        Map<Student, Set<Subject>> students = Map.of(
                new Student("Student1", "000001", "201-18-15"),
                Set.of(new Subject("Math", 70),
                        new Subject("English", 85)),
                new Student("Student2", "000002", "201-18-15"),
                Set.of(new Subject("Economic", 75),
                        new Subject("Sociology", 65))
        );
        Collage collage = new Collage(students);
        assertThat(collage.findBySubjectName(
                "000001", "Sociology")).isEqualTo(Optional.empty());
    }

    @Test
    public void whenSubjectIsOptionalNotEmpty() {
        Map<Student, Set<Subject>> students = Map.of(
                new Student("Student1", "00001", "201-18-15"),
                Set.of(new Subject("Math", 70),
                        new Subject("English", 85)),
                new Student("Student2", "00002", "201-18-15"),
                Set.of(new Subject("Economic", 75),
                        new Subject("Sociology", 65))
        );
        Collage collage = new Collage(students);
        assertThat(collage.findBySubjectName(
                "00002", "Sociology").get().getScope()).isEqualTo(65);
    }
}